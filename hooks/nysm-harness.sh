#!/bin/bash
# nysm harness — active development acceleration hooks
...
if echo "$PROMPT" | grep -qE '^\./scope'; then
  ...
  if [ -n "$SCOPE" ]; then
    mkdir -p "$HOME/.nysm"
    echo "$SCOPE" > "$HOME/.nysm/scope.lock"
    cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[nysm harness: scope locked]\nScope restricted to: ${SCOPE}\nDo NOT modify any files outside this scope. If you need to read files outside scope, that's fine. But all Edit/Write operations must target files within: ${SCOPE}\nTo unlock: ./unlock"}}
EOJSON
  else
    cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[nysm harness] Usage: ./scope src/auth/ — restricts edits to that directory"}}
EOJSON
  fi
  exit 0
fi
...
if echo "$PROMPT" | grep -qE '^\./unlock'; then
  rm -f "$HOME/.nysm/scope.lock"
  cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[nysm harness: scope unlocked] No restrictions on file modifications."}}
EOJSON
  exit 0
fi
...
if echo "$PROMPT" | grep -qE '^\./relax'; then
  rm -f "$HOME/.nysm/strict.lock"
  cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[nysm harness: strict mode OFF] Guardrails relaxed."}}
EOJSON
  exit 0
fi
