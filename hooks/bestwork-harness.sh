...
if echo "$PROMPT" | grep -qE '^\./scope'; then
  ...
  if [ -n "$SCOPE" ]; then
    ...
    cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[BW: scope locked]\nScope restricted to: ${SCOPE}\nDo NOT modify any files outside this scope. If you need to read files outside scope, that's fine. But all Edit/Write operations must target files within: ${SCOPE}\nTo unlock: ./unlock"}}
EOJSON
  else
    cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[BW] Usage: ./scope src/auth/ — restricts edits to that directory"}}
EOJSON
  fi
  exit 0
fi
...
if echo "$PROMPT" | grep -qE '^\./unlock'; then
  ...
  cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[BW: scope unlocked] No restrictions on file modifications."}}
EOJSON
  exit 0
fi
...
if echo "$PROMPT" | grep -qE '^\./relax'; then
  ...
  cat <<EOJSON
{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[BW: strict mode OFF] Guardrails relaxed."}}
EOJSON
  exit 0
fi
