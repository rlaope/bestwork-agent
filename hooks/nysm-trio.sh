#!/bin/bash
# nysm trio — parallel task execution with Tech + PM + Critic per task
# Each task gets 3 agents: developer, product manager, critic

INPUT=$(cat)
PROMPT=$(echo "$INPUT" | jq -r '.prompt // .display // ""' 2>/dev/null)

if ! echo "$PROMPT" | grep -qE '^\./trio' && [ "$NYSM_TRIO_TRIGGER" != "1" ]; then
  echo '{}'
  exit 0
fi
