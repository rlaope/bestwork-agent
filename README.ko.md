...
**bestwork-agent**는 에이전트한테 팀을 붙여줍니다. 태스크마다 **Tech**(개발) + **PM**(검증) + **Critic**(리뷰) 3명이 붙어서 일합니다. 36개 전문 프로필 자동 매칭. 병렬 실행. 피드백 루프. 디스코드/슬랙 알림.
...
Claude Code 재시작하고 `./help` 치면 전체 명령어 목록 나옵니다.

또는 Claude Code 안에서 바로:
```
./bw-install
...
**Tech 18개**: backend, frontend, fullstack, infra, database, API, mobile, testing, security, performance, devops, data, ML, CLI, realtime, auth, migration, config

**PM 8개**: product, API, platform, data, infra, migration, security, growth

**Critic 10개**: performance, scalability, security, consistency, reliability, testing, hallucination, DX, type safety, cost
